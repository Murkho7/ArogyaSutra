...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        with init_context(), warnings.catch_warnings():
            ...
            # Fix all sub-configs that might be dicts
            if hasattr(actor_model_config, 'text_config') and isinstance(actor_model_config.text_config, dict):
                from transformers import Qwen2Config
                actor_model_config.text_config = Qwen2Config(**actor_model_config.text_config)
    
            if hasattr(actor_model_config, 'vision_config') and isinstance(actor_model_config.vision_config, dict):
                try:
                    from transformers import Qwen2VLVisionConfig
                    actor_model_config.vision_config = Qwen2VLVisionConfig(**actor_model_config.vision_config)
                except ImportError:
                    # Fallback if the specific config class doesn't exist
                    from transformers import PretrainedConfig
                    actor_model_config.vision_config = PretrainedConfig(**actor_model_config.vision_config)
