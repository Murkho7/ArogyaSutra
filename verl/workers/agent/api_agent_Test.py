    ...
    def generate_conversation(self, system_prompt, initial_prompt, images=None):
        ...
        for turn in range(self.max_turns):
            ...
            try:
                ...
                # if ((not has_action and self.extract_boxed_answer(generated_text)) 
                #     or (turn == self.max_turns - 1)):
                #     break
                # if not has_action and not self.extract_boxed_answer(generated_text) and not self.code_sw:
                #     self.code_sw=True
                #     obs_text = "Degenerative language detected. Try to answer in English."
